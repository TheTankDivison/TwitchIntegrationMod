--[[
function ChatGui:receive_message(name, message, color, icon, twitch)
...
function ChatGui:receive_message(name, message, color, icon)
	...
	if  #list_of_files_before ~= #ChatGui.list_of_files then
		...
		for i = 1, #list_of_files_before, 1 do
			...
			ChatGui.list_of_files[gs] = "mods/TwitchIntegrationMod/ReadMessage/emotes/"..gs
		end
	end
...
--[[
Hooks:PostHook(ChatGui, "_layout_output_panel", "_layout_output_panel_TW", function(self)
	for i = #ChatGui._lines, 1, -1 do
		local line1 = ChatGui._lines[i][1]
		for j, elem in ipairs(self._lines[i]) do
			
			local Emote = ChatGui._lines[i][j+3]
			
			if Emote then
				if Emote:name()=="twitch_icon" then
					Emote:set_top(line1:top() + 2.5)
					Emote:set_left(Emote:left())
				else
					local n = tonumber(Emote:name())
					
					Emote:set_top(line1:top() + (ChatGui.line_height-2)*(n-1))
				end
